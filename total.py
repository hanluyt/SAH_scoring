...
if __name__ == "__main__":
    ...
    train_data = pd.read_csv('sah_train.csv')
    test_data = pd.read_csv('sah_test.csv')
    ...
    # plt.title('ROC curve of valid data')
    # plt.plot(fpr_svm_t, tpr_svm_t, color='red', label='svm AUC = %0.3f' % df_scores_valid.mean()['svm_auc'])
    # plt.plot(fpr_lr_t, tpr_lr_t, color='orangered', label='lr AUC = %0.3f ' % df_scores_valid.mean()['lr_auc'])
    # plt.plot(fpr_rf_t, tpr_rf_t, color='brown', label='rf AUC = %0.3f' % df_scores_valid.mean()['rf_auc'])
    # plt.legend()
