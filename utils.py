...
class Rater():
    ...
    Parameters
    ----------
    metric :    {'aucprc', 'mcc', 'fscore', 'auc'}, optional (default='auc')
    ...
    """
    def __init__(self, metric='auc', threshold=0.5):

        if metric not in ['aucprc', 'mcc', 'fscore', 'bacc', 'auc']:
            raise ValueError(f'Metric {metric} is not supported.\
                \nSupport metrics: [aucprc, mcc, fscore, auc].')
